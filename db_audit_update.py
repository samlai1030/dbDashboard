"""
db_audit_update.py — Sync files from audit_datasets/ into a SFR_audit table in data.db.

• All audit files merge into ONE table (SFR_audit).
...
Usage:
    python db_audit_update.py              # import new files only
    python db_audit_update.py --dry-run    # preview what would be imported
    python db_audit_update.py --rebuild    # drop everything & re-import from scratch
    python db_audit_update.py --status     # show import log & row counts
"""
...
DATASETS_FOLDER = cfg.audit_datasets
DB_PATH = cfg.db_file

TABLE_NAME = "SFR_audit"
...
def _existing_columns(conn: sqlite3.Connection) -> set[str]:
    """Return set of column names in SFR_audit, or empty set if table doesn't exist."""
    try:
...
def sync_datasets(
    ...
    *,
    ...
    if not files:
        print("No data files found in audit_datasets/")
        return
    ...
    if rebuild:
        if not dry_run:
            ...
            conn.commit()
            print("Dropped SFR_audit table.\n")
        else:
            print("[DRY] Would drop SFR_audit table.\n")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Sync audit_datasets/ into SFR_audit table in data.db"
    )
    parser.add_argument(
        "--src", type=str, default=str(DATASETS_FOLDER), help="Audit datasets folder"
    )
    parser.add_argument(
    ...
    src = Path(args.src)
    if not src.exists():
        print(f"Error: audit_datasets folder not found: {src}")
        return
    ...
    if args.dry_run:
        print("[DRY RUN]")
    if args.rebuild:
        print("[REBUILD — dropping SFR_audit first]")
    print()
